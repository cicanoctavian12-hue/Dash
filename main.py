...
async def code(ctx, member1: discord.Member, member2: discord.Member, member3: discord.Member = None, member4: discord.Member = None, *, code: str):
    ...
    if member3 and member4 and code:
        try:
            ...
            await member1.send(f"Your room code vs {team1_opponents} is:\n```{code}```")
            await member2.send(f"Your room code vs {team1_opponents} is:\n```{code}```")
            await member3.send(f"Your room code vs {team2_opponents} is:\n```{code}```")
            await member4.send(f"Your room code vs {team2_opponents} is:\n```{code}```")
...
async def ticket(ctx):
    ...
    embed = discord.Embed(
        description="Need support? Open a ticket by clicking one of buttons.",
